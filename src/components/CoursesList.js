    ...
    <div className="bg-[#34495E] p-8 rounded-lg shadow-lg space-y-8">
      ...
      <div className="flex justify-start mb-6">
        <div className="relative w-full max-w-lg">
          <FaSearch className="absolute left-4 top-3 p-1 text-white-400 bg-[#3FA2F6] h-6 w-6 rounded" />
          <input
            type="text"
            className="w-full p-3 pl-12 bg-[#2C3E50] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3FA2F6] placeholder:text-gray-300"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-8">
        {filteredCourses.length === 0 ? (
          ...
          filteredCourses.map((course) => (
            <div
              key={course.id}
              className="bg-[#2C3E50] p-6 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105"
            >
              {/* Course Header */}
